# Backward sampling function, takes the proposal mass function to generate a 
# proposal for a chain.

backward_sampling <- function(proposal_pinfected, pen_states, id_number, id_pen, 
  ...
  proposal_state <- rep(0, trials)
  
  proposal_like <- rep(0, trials)
  
  proposal_state[trials] <- rbinom(n = 1, size = 1, prob = proposal_pinfected[trials])
  ...
  results$proposal_state <- proposal_state
  
  results$loglike <- sum(log(proposal_like))
